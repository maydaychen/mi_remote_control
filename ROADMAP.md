...
- 2026-09-22 08:01 Harness task 门禁通过：47 个维护文件规模检查未扩大历史 baseline，最终构建与完整内置自检全部通过，保留 1 条既有 Swift 闭包捕获 warning。测试 App 实测 automatic 将默认输入从 MacBook Pro Microphone（设备 98）临时切到 BlackHole 2ch（设备 103）并约 1.35 秒后恢复，manual 全程保持设备 98；两种模式均从 BlackHole 双声道录回 1 kHz 测试音，峰值 -18.00 dBFS、RMS 约 -21.1 dBFS。ffmpeg 的 AVFoundation 录制在 20.02 秒墙钟时间只写入约 17.38 秒文件时间，按同次捕获时钟归一化后测试音约 1.02 秒；源码自检同时断言源 PCM 恰为 16000 个 16 kHz 样本。统计关闭后测试音计数保持 6，重启后计数、开启状态与 automatic 偏好均读回。当前输入监控／辅助功能未授权，真实遥控器两种路由语音、Home／TV 精确计数、清空确认及完整浅色／最小窗口／VoiceOver 人工验收尚待完成。

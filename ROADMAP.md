...
- 2026-09-22 00:49 将按键映射、场景配置、语音和通用页统一改为固定标题区与独立滚动正文；标题区按 AppKit 窗口实际装饰高度动态避让 macOS 统一标题栏。
...
- 2026-09-22 00:49 无引擎 UI 预览在副屏定向截图确认“按键映射”标题、副标题、“识别按键”和分隔线均完整位于系统标题栏下方；源码结构确认标题区在 `ScrollView` 外，构建与内置自检通过。

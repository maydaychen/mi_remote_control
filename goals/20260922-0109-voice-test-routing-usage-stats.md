...
- 状态：实现完成，验收进行中；自动化、BlackHole 集成、统计开关、重启持久化与确认清空、浅色／深色最小窗口 UI、带数据辅助功能文本及 Reduce Motion 代码路径已通过，真实遥控器和键盘焦点人工验收待完成。
...
### 当前验证快照（2026-09-22 08:56，北京时间）

- 已实现 T1～T5；`./build.sh`、`.build/miremote --self-test` 与 Harness task 门禁全部通过，47 个维护文件规模检查未扩大历史 baseline。
- automatic 实测将默认输入从设备 98 临时切到 BlackHole 设备 103，并约 1.35 秒后恢复；manual 全程保持设备 98。
- 两种模式均从 BlackHole 双声道录回 1 kHz 测试音，峰值 -18.00 dBFS、RMS 约 -21.1 dBFS；ffmpeg AVFoundation 捕获存在稳定文件时钟缩短，按同次 20.02 秒墙钟／17.38 秒文件时长归一化后测试音约 1.02 秒，源码自检另断言源 PCM 恰为 16000 个 16 kHz 样本。统计关闭后测试音计数不增加，重启后计数、开关与路由偏好可读回。
- 浅色与深色模式均已检查语音页和统计页；设置窗口现有 760×560 最小内容尺寸，两页在该尺寸下保持纵向滚动且无整体横向溢出。辅助功能树可读出今日三张摘要卡；点击“清空…”后选择取消，页面数据保持不变。
- 隔离临时用户目录中的 7 天样本验证了有数据状态：辅助功能树准确输出“今日遥控动作 4 次”“今日语音 2 次”“语音时长 1 分 40 秒”和唯一一段 7 天趋势摘要；期间未读取或改写真实统计文件。
- Reduce Motion 分支由内置自检断言：开启时取消实时电平表动画，关闭时保留；测试音完成／失败和统计变化均以静态文字、图标与数值表达，不依赖动画或单一颜色。
- 测试音服务自检直接覆盖 busy、缺输出设备、路由失败、音频引擎失败和成功路径：失败均不增加成功计数；成功写入 16000 样本、自动还原输入并只计 1 次。MappingEngine 自检分别覆盖 tap／hold／double／gesture 各只记录一次，统计自检明确覆盖零帧语音不创建会话。
- 与当前提交一致的 `dist/MiRemote-Test.app` 已生成，从该 App 内执行 `--doctor` 确认辅助功能、输入监控、遥控器连接、BlackHole 与 hidutil 残留检查全部通过。待完成：两种路由各 2 次真实遥控器语音、Home／TV 与语音的精确计数、确认清空，以及完整键盘焦点人工验收；这些步骤需要实际按遥控器、说话、确认删除或切换系统键盘导航，尚不能标为已通过。
- 2026-09-22 11:16 使用最终测试 App 复验真实本地统计：关闭“记录本地使用统计”后播放一次测试音，页面与磁盘计数均保持 9；重新开启后播放一次，测试音变为 10，遥控动作、语音会话和语音时长继续为 0；正常退出并重新打开后，计数 10 与开关开启状态均成功读回。清空确认框已显示准确的永久删除范围，但尚未执行最终“清空”动作。
- 2026-09-22 11:19 经用户确认后执行“清空全部使用统计”：页面中的遥控动作、语音、语音时长和测试音立即全部归零，`usage-stats.json` 被删除；正常退出并重新打开后仍为全零，统计开关保持开启，证明确认清空与空状态持久化均符合预期。

### 当前验收审计

| 文档标准 | 当前证据 | 状态 |
| --- | --- | --- |
| 9.1 自动化验收 | 生成参数、路由真值表、测试音成功／失败、统计聚合／持久化／损坏恢复／清空、逻辑动作单计数、零帧与多帧语音均由完整自检覆盖；Harness task 全绿且未扩大 baseline | 已通过 |
| 9.2A 自动路由测试音 | 默认输入 98→103→98，约 1.35 秒恢复；BlackHole 录回 1 kHz、-18.00 dBFS，归一化时长约 1.02 秒；测试音与语音计数隔离 | 已通过 |
| 9.2B 手动路由测试音 | 默认输入前中后保持设备 98，BlackHole 录回信号参数合格；目标语音工具电平仍需现场观察 | 部分通过 |
| 9.2C 真实遥控器语音 | 权限、遥控器和 BlackHole 前置已通过；两种模式各 2 次连续语音与文字结果尚无本轮证据 | 待人工 |
| 9.2D 统计持久化与精确计数 | 自动化覆盖计数、禁用、读回与清空；真实 App 已验证关闭后不计数、开启后准确 +1、测试音不混入语音、重启读回、确认清空与再次重启保持全零；仅固定 Home／TV／语音序列需随遥控器真机验收补测 | 部分通过 |
| 9.3 UI 与可访问性 | 浅／深色、760×560、VoiceOver 等价文本和 Reduce Motion 已通过；键盘全控焦点环待临时启用系统键盘导航后检查 | 部分通过 |

现场净增量基线（2026-09-22 11:19，北京时间）：确认清空并重启后，真实统计中今日遥控动作、语音会话、语音样本和测试音均为 0，统计文件不存在，统计开关保持开启；系统 `AppleKeyboardUIMode=1`。后续执行 9.2C／9.2D 的遥控器步骤时以全零为起点比较净增量；键盘焦点验收前需把系统键盘导航临时切到全控，完成后恢复原值 1。
